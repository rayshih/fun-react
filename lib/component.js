  ...
  const cycleDefFn = (interactions, props, self, lifecycles) => {
    ...
    const registerEvent = eventType => {
      ...
      if (!eventTypes.includes(etn)) {
        eventTypes.push(etn)
        forwardEvent(etn, interactions.get(etn))
      }
    ...
    const mapAndLinkEventWithObj = (obj, element) => {
      ...
      return mapAndLinkEvent(mapper, element)
    }

    const mapAndLinkOrdinaryEvent = (eventMap, element) => {
      const mapper = evt => eventMap[evt.eventType](evt.payload)
      return mapAndLinkEvent(mapper, bridge(element, ...Object.keys(eventMap)))
    }

    registerEvent.link = linkEvent
    registerEvent.map = mapAndLinkEvent
    registerEvent.mapWithObj = mapAndLinkEventWithObj
    registerEvent.mapOrdinary = mapAndLinkOrdinaryEvent
